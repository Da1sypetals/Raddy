...
def scalar_matrix_mul_template(scalar_ref, matrix_ref):
    ...
    return f"""
impl<const N: usize, const R: usize, const C: usize> Mul<{matrix}SMatrix<Scalar<N>, R, C>> for {scalar}Scalar<N> {{
    type Output = SMatrix<Scalar<N>, R, C>;

    fn mul(self, rhs: {matrix}SMatrix<Scalar<N>, R, C>) -> Self::Output {{
