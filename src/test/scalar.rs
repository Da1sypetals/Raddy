...
use na::ComplexField;
use na::{Const, DimName, Matrix3, SMatrix, SVector, U3, U5};
...
fn test_scalar3() {
    ...
    for test_it in 0..100 {
        ...
        let g = y.grad()[(0, 0)];
        let h = y.hess()[(0, 0)];
        assert_abs_diff_eq!(g, grad_001(sv), epsilon = EPS);
        assert_abs_diff_eq!(h, hess_001(sv), epsilon = BIG_EPS);
        ...
        let g = y.grad()[(0, 0)];
        let h = y.hess()[(0, 0)];
        assert_abs_diff_eq!(g, grad_002(sv), epsilon = EPS);
        assert_abs_diff_eq!(h, hess_002(sv), epsilon = BIG_EPS);
        ...
        let g = y.grad()[(0, 0)];
        let h = y.hess()[(0, 0)];
        assert_abs_diff_eq!(g, grad_003(sv), epsilon = EPS);
        assert_abs_diff_eq!(h, hess_003(sv), epsilon = BIG_EPS);
    }
}
